...
class TripCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (now.isAfter(trip.endDate) || now.isAtSameMomentAs(trip.endDate)) {
      ...
      progress = now.difference(trip.startDate).inDays / duration;
    }
    ...
    final progressColor = isCompleted ? Colors.green.shade200 : AppColors.primary.withOpacity(0.5);
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: LivestTypography.bodySm.copyWith(color: AppColors.textSecondary),
                      ),
                      const SizedBox(height: 8),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
                        decoration: BoxDecoration(
                          color: AppColors.primary.withOpacity(0.5), // Match badge color from design (blueish)
                          borderRadius: BorderRadius.circular(12),
                        ),
                        child: Text(
                          '$duration Hari',
                          style: LivestTypography.captionBold.copyWith(color: AppColors.baseWhite),
                        ),
                      )
                    ],
                  ),
